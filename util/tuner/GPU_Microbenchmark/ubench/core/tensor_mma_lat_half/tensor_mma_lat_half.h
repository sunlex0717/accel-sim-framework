...
__global__ void tensor1688_latency<half,half>(uint64_t *startClk, uint64_t *stopClk, half *input_A,
  ...
  int gid = blockIdx.x * blockDim.x + threadIdx.x;
  /** step 0: create shared memory buffer, this step is  necessary because we need to use ldmatrix instruction which can only load from shared mem **/
   __shared__ half smem_buffer_A[16*8]; // wmma_m * wmma_k/2 : Sparse Matrix A
   __shared__ half smem_buffer_B[8*8]; // wmma_n * wmma_k : Dense Matrix B
  // register T result = 0;
  /** step 0.5: load from gloabl to shared **/
...
template <typename T, typename R>
__global__ void tensor884_latency(uint64_t *startClk, uint64_t *stopClk, T *input_A,
                               T *input_B, R *output_D ){
...
template <>
__global__ void tensor884_latency<half,float>(uint64_t *startClk, uint64_t *stopClk, half *input_A,
                               half *input_B, float *output_D ) {

  int gid = blockIdx.x * blockDim.x + threadIdx.x;
  /** step 0: create shared memory buffer, this step is  necessary because we need to use ldmatrix instruction which can only load from shared mem **/
   __shared__ half smem_buffer_A[8*4]; // wmma_m * wmma_k/2 : Sparse Matrix A
   __shared__ half smem_buffer_B[4*8]; // wmma_n * wmma_k : Dense Matrix B
  // register T result = 0;
  /** step 0.5: load from gloabl to shared **/
  ...
  if(threadIdx.x == 0){//we do not care global->shared memory when profiling tensor core, so no need for thread-cooperation
    ...
    /** step 1: create register for each thread **/
  half frag_A[4]; // two .f16x2 registers, 8 half elements, 
  half frag_B[4];  // two .f16x2 registers, 8 half  elements
  float frag_D[8]; //result(fp32) 8 f32 registers
...
template <>
__global__ void tensor884_latency<half,half>(uint64_t *startClk, uint64_t *stopClk, half *input_A,
                               half *input_B, half *output_D ) {
  ...
  half frag_A[4]; // two .f16x2 registers, 8 half elements, 
  half frag_B[4];  // two .f16x2 registers, 8 half  elements
  half frag_D[8]; //result(fp32) 8 f32 registers
...
template <class T, class R> float tensor884_lat() {
  int mma_m = 8;
  int mma_n = 8;
  int mma_k = 48;
